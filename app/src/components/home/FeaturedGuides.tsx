              ...
              <article className="home-guide-card" key={guide.id}>
                <div className="home-guide-photo">
                  {!imageErrors.includes(guide.id) ? (
                    <Image src={guide.avatar} alt={`${guide.displayName}, a local guide in ${guide.city}`} fill sizes="(max-width: 640px) 82vw, (max-width: 1024px) 46vw, 310px" onError={() => setImageErrors((ids) => [...ids, guide.id])} />
                  ) : (
